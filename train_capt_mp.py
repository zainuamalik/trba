# =================================================================================================
# TRAINING LOGIC WITH MIXED PRECISION, CHARACTER ERROR RATE (CER), LEARNING RATE AND NEW FORMATTING 
...
def train(opt):
    ...
    # Weight initialization with mixed precision considerations
    for name, param in model.named_parameters():
        ...
        try:
            if 'bias' in name:
                init.constant_(param, 0.0)  # Initialize bias to 0
            elif 'weight' in name:
                if param.ndimension() > 1:  # Check if the parameter is a matrix (weights in layers)
                    init.kaiming_normal_(param, mode='fan_out', nonlinearity='relu')  # Kaiming initialization
                else:
                    init.normal_(param, mean=0.0, std=0.01)  # For other cases, use normal distribution
        except Exception as e:  # For batchnorm or other layer types
            if 'weight' in name:
                param.data.fill_(1)  # Batchnorm weights initialized to 1
            continue
        continue
    ...
    while True:
        ...
        if 'CTC' in opt.Prediction:
            cost = criterion(
                preds.float(),  # Convert predictions to FP32
                text.float() if text.dtype != torch.int else text,  # Handle text precision
                preds_size.float(),  # Convert preds_size to FP32
                length.float() if length.dtype != torch.int else length  # Handle length precision
            ) / batch_size
        else:
            preds = model(image, text[:, :-1])  # align with Attention.forward
            target = text[:, 1:]  # without [GO] Symbol
            cost = criterion(
                preds.view(-1, preds.shape[-1]).float(),  # Convert predictions to FP32
                target.contiguous().view(-1).long()  # Convert target to torch.int64 (long)
            )
        ...
        # Track loss
        loss_avg.add(cost.detach())  # Use `.detach()` to avoid holding computational graph references

        # Apply the learning rate scheduler
        if opt.scheduler == 'step' or opt.scheduler == 'cosine':
            scheduler.step()  # Step the scheduler after each iteration for step/cosine scheduling
        ...
        if (iteration + 1) % opt.valInterval == 0 or iteration == 0:
            ...
            with open(f'./saved_models/{opt.exp_name}/log_train.txt', 'a') as log:
                ...
                # Reset train loss averager
                loss_avg.reset()

                

                # Save best accuracy and norm_ED models
